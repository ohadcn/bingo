...
lengths = WordsDict(initlen=1, db_file='duckduckgo.pickle')
...
class Proxy(threading.Thread):
    ...
    def __init__(self, client):
        ...
        self.timer = Timer(1.0, self.nextWord)
    ...
    def _process_request(self, data):
        ...
        if self.server and not self.server.closed:
            ...
            logger.debug('sending packet of size %d to %s', len(data), self.server.addr[0])
            return
...
#                 raise ProxyConnectionFailed(host, port, repr(e))
            ...
            if self.request.method == b"CONNECT" and host.startswith('ac.duckduckgo.com'):
                logger.debug('sending connect request of size %d to %s', len(data), host)
    ...
    def _process_response(self, data):
        ...
        self.total_recived_length = len(data)
        if self.server.addr[0].startswith('ac.duckduckgo.com') and self.server.addr[1] == 443:
            logger.debug('got response of size %d from %s, total %s', len(data), self.server.addr[0], self.total_recived_length)
            if lengths.block:
                duckduckgo.LAST_LENGTH = self.total_recived_length
            else:
                self.timer.cancel()
                self.timer = Timer(1.0, self.nextWord)
                self.timer.start()
    ...
    def nextWord(self):
        try:
            self.word = lengths.getSimilar(self.total_recived_length, self.word)[0]
        except:
            logger.critical('got response of size %d, \033[1;31m%s\033[0m'%(self.total_recived_length,'failed to get query word'))
            return
        logger.critical('got response of size %d, search query is \033[1;31m%s\033[0m'%(self.total_recived_length,self.word))
        self.total_recived_length = 0
...
#             LAST_LENGTH = self.total_recived_length
